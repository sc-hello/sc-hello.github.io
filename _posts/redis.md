...
#  redis 内存淘汰机制(MySQL里有2000w数据，Redis中只存20w的数据，如何保证Redis中的数据都是热点数据?)+1
redis提供了8种数据淘汰策略：
...
# 如何保证缓存与数据库双写时的数据⼀致性？+1
一般来说，如果允许缓存可以稍微跟数据库偶尔有不一致的情况，也就是说系统不是严格要求 “缓存+数据库” 必须保持一致性的话，最好不要做这个方案，即：读请求和写请求串行化，串到一个内存队列里去。
