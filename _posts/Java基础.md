...
# 7 如果两个对象的hashcode相同，equals()一定为true吗？+2
...
# 8 为什么equals()重写的话，建议也一起重写hashcode方法？+2
...
# String,StringBuffer,StringBuilder的区别+2
...
# public,protected,default,private的访问权限+2
...
2. **new**：会创建1或2个对象，如果String Pool中没有“abc”，编译期在String Pool中创建一个"abc"对象，运行期使用new的方式在堆中创建另外一个"abc"对象，并返回该对象的引用；如果String Pool中已有一个"abc"对象，则编译期不创建，运行期使用new的方式在堆中创建另外一个"abc"对象，并返回该对象的引用；
# 20 浅克隆和深克隆的区别+2
...
# 21 new Integer(123) 与 Integer.valueOf(123)有何区别，请从底层实现分析两者区别+2
...
4. JSP+JavaBean 中 JSP 用于处理用户请求，JavaBean 用于封装和处理数据。该模式只有视图和模型，一般把控制器的功能交给视图来实现，适合业务流程比较简单的 Web 程序。JSP+JavaBean 模式在一定程度上实现了 MVC，即 JSP 将控制层和视图合二为一，JavaBean 为模型层；
...
# 31 Classloader.loadClass(className)和Class.forName(className)的区别+2
...
3. 使用：3.1java.lang.reflect包中的三个类（1Field:成员变量 2Method:成员方法 3Constructor:构造方法）3.2对public域的方法:getField、getMethod、getConstructor 3.3对所有域的方法:getDeclaredField、getDeclaredMethod、getDeclaredConstructor 3.4利用反射访问私有属性:使用setAccessible(true)；
...
# 37 Java中的String为什么不可变？不可变的好处？+2
