...
4. 1 多个socket可能会并发产生不同的事件，I/O多路复用程序会监听多个socket，会将socket放入一个队列中排队；2 I/O多路复用程序每次从队列中取出一个socket给文件事件分派器，文件事件分派器根据socket产生的事件交给对应的事件处理器进行处理；3 一个socket的事件处理完之后，I/O多路复用程序才会将队列中的下一个socket给到文件事件分派器；
